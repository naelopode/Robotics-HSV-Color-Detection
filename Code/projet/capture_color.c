...
#include <chprintf.h>
#include <usbcfg.h>
#include <leds.h> //enlever ?
#include <main.h> //enlever ?
...
//Local typdef
typedef struct{ //RGB as usigned long int as it can reach 252 000
	unsigned long int red;
	unsigned long int green;
	unsigned long int blue;
}color_rgb_long_t;

typedef struct{ //HSV color
	float hue;
	float saturation;
	float value;
}color_hsv_t;
...
//Local function
void RGB2HSV(color_rgb_n_t, color_hsv_t *output);
void print_color(color_rgb_n_t, color_hsv_t, color_type); //function for debug purposes
...
static THD_FUNCTION(ProcessImage, arg) {
    ...
    chRegSetThreadName(__FUNCTION__);
    (void)arg;
    static color_rgb_long_t color_rgb_long =  {0,0,0};
...
void RGB2HSV(color_rgb_n_t input, color_hsv_t *output){ //Code from https://www.tutorialspoint.com/c-program-to-change-rgb-color-model-to-hsv-color-model
...
void print_color(color_rgb_n_t input_RGB, color_hsv_t input_HSV, color_type format){ //This function print values ! This is for debug purposes
	switch (format){
		case RGB:
